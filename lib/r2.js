...
// Build a Content-Disposition that names the object exactly `name`. inline → the PDF opens in the
// browser and saves under this name (preview-then-download); otherwise it downloads immediately. RFC
// 5987 filename* carries any unicode; filename="" is the ASCII fallback. Header-breaking chars stripped.
...
// When downloadName is given, the presigned URL carries a signed ResponseContentDisposition so the
// browser saves the object under that name — the cross-origin <a download> attribute is ignored, so
// this header is the only way to control the saved filename. Omit downloadName to open inline unnamed.
export function presignGet(key, downloadName, inline) {
  return getSignedUrl(
    r2,
    new GetObjectCommand({
      Bucket: R2_BUCKET,
      Key: key,
      ...(downloadName ? { ResponseContentDisposition: contentDisposition(downloadName, inline) } : {}),
    }),
    { expiresIn: 300 }
  );
}
...
export const UPLOAD_LIMITS = {
  task:       { exts: ["pdf"], maxBytes: 25 * 1024 * 1024 },
