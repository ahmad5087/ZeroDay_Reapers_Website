// Country reference for signup/profile: ISO-3166 alpha-2 code, display name, and the
// international dialing code. The flag emoji is DERIVED from the code (regional-indicator
// letters) so we never hardcode ~200 emoji. Used by AuthScreen (signup), ProfileScreen
// (phone edit), and everywhere a member's name is shown (flag after the name).
...
// ISO alpha-2 → flag emoji (two regional-indicator symbols). "PK" → 🇵🇰.
export function flagFor(code) {
  const cc = (code || "").toUpperCase().replace(/[^A-Z]/g, "").slice(0, 2);
  if (cc.length !== 2) return "";
  return String.fromCodePoint(...[...cc].map((c) => 127397 + c.charCodeAt(0)));
}

const BY_CODE = Object.fromEntries(COUNTRIES.map((c) => [c.code, c]));
export const countryByCode = (code) => BY_CODE[(code || "").toUpperCase()] || null;
export const dialFor = (code) => countryByCode(code)?.dial || "";
export const countryNameFor = (code) => countryByCode(code)?.name || "";
